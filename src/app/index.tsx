...
import { BasePageConfig, PublicationPageConfig, TextPageConfig, CardPageConfig, BlogPageConfig, BlogPost, ProjectsPageConfig, ProjectItem } from '@/types/page';
import { getRuntimeI18nConfig } from '@/lib/i18n/config';

interface SectionConfig {
  id: string;
  type: 'markdown' | 'publications' | 'list' | 'blog' | 'projects';
  title?: string;
  source?: string;
  filter?: string;
  limit?: number;
...
function processSections(sections: SectionConfig[], locale?: string): SectionConfig[] {
  return sections.map((section: SectionConfig) => {
    switch (section.type) {
      ...
      case 'blog': {
        const blogData = getPageConfig<BlogPageConfig>('blog', locale);
      ...
      case 'projects': {
        const projectsData = getPageConfig<ProjectsPageConfig>('projects', locale);
        const allProjects = projectsData?.projects || [];
        return {
          ...section,
          projects: allProjects.slice(0, section.limit || 5),
        };
      }
      default:
        return section;
...
function loadPageDataForLocale(locale: string | undefined): HomePageLocaleData {
  ...
  const enableOnePageMode = localeConfig.features?.enable_one_page_mode ?? false;

  const aboutConfig = getPageConfig<{ profile?: { research_interests?: string[]; career_interests?: string[] }; sections?: SectionConfig[] }>('about', locale);
  const researchInterests = aboutConfig?.profile?.research_interests;
  const careerInterests = aboutConfig?.profile?.career_interests;
